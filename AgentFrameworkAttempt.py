...
class Agent:
    ...
    def __init__ (self, environment, agents, y, x): # Passes variables into a list to link to other objects

        self._x = random.randint(0,99) # Set starting x co-ordinate to somewhere between 0 and 99
        self._y = random.randint(0,99) # Set starting y co-ordinate to somewhere between 0 and 99
    ...
    def move (self): # Create a method to make the agents move
        ...
        if random.random() < 0.5: # If random number is less than 0.5
            self.y = (self._y + 1) % 100 # Add 1 to x co-ordinate
        else: # If random number is not less than 0.5
            self.y = (self._y - 1) % 100 # Subtract 1 from x co-ordinate

        if random.random() < 0.5: # If random number is more than 0.5
            self.x = (self._x + 1) % 100 # Add 1 to y co-ordinate
        else: # If random number is not less than 0.5
            self.x = (self._x - 1) % 100 # Subtract 1 from y co-ordinate
    ...
    def eat(self): # Create a method to make the agents (sheep) eat 
        ...
        if self.environment[self._y][self._x] > 10: # If the environment (field) is more than 10
            self.environment[self._y][self._x] -= 10 # Minus 10 from the environment (field)
    ...
    def distance_between(self, agent): # Define distance and work out distance between two sheep
        ...
        return (((self._x - agent.x)**2) + ((self._y - agent.y)**2))**0.5  # Pythagoras Theorem
